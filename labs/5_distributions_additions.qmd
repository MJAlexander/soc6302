...
To calculate the z-scores, we just follow the standardization formula given in the slides. 

$$
Z  = \frac{X - \mu}{\sigma}
$$

Can define these as new variables. NOTE: order of operations means we need to put parentheses around the numerator:
...
- Closest to the mean: Monica
- Interpretation: Teddy's height is 10.375 standard deviations lower than the mean
- Unlikely that he's a full-grown adult
...
```{r}
pnorm(q = r_height, mean = mu, sd = sigma) - pnorm(q = m_height, mean = mu, sd = sigma)
```

-   Calculate the probability of observing a height that is greater than Teddy's

```{r}
1-pnorm(t_height, mean = mu, sd = sigma)
```
...
Now let's simulation 100 heights, and save these in a tibble.

```{r}
set.seed(541)
sample_heights <- tibble(person_number = 1:100, # from 1 to 100
...
```{r}
sample_heights_2 <- tibble(person_number = 1:100, 
                           height = rnorm(100, mean = mu, sd = sigma))
```
...
```{r}
sample_heights <- sample_heights |> mutate(sample_number = 1) #first set of samples
sample_heights_2 <- sample_heights_2 |> mutate(sample_number = 2) # secon set of sample
...
sample_heights <- tibble() # define an empty data frame to store our results in

for(i in 1:1000){ # for i going to 1 to 1000
  
  # creating a data frame with 100 random samples and sample number column
  this_sample <- tibble(person_number = 1:100, 
                        height = rnorm(100, mean = mu, sd = sigma), 
                        sample_number = i)
  # take this new sample, and add it on to our existing samples
...
```{r}
ggplot(data = mean_heights, aes(mean))+
  geom_histogram()
```
